...
7. After that final compile, use `BlueprintTools.get_default_object` on the WidgetBlueprint, then ObjectTools `list_properties -> get_properties -> set_properties` on the returned generated `UUserWidget` CDO. Set only `designSizeMode` to the explicit interface-analysis decision (`FillScreen|Desired`). Do not recompute it from `assetKind`, path, or an `umg_`/`uw_` prefix. A legacy/standalone plan without an explicit decision uses its recorded `fallback-unclear -> FillScreen` result.
