...
- A complete screen also records `profile.designSizeMode: FillScreen`; a child widget or collection entry records `profile.designSizeMode: Desired`. The mode controls the Widget Blueprint Designer `Screen Size` preview and is derived from the reviewed asset kind. It neither replaces `referenceSize` nor authorizes `DesignTimeSize` writes.
...
- `designSizeMode`: `FillScreen` or `Desired`. New layouts must set it explicitly. `screen` requires `FillScreen`; `child-widget` requires `Desired`. The `prototype` kind is archived compatibility only and must not be newly emitted; if an archived prototype already supplies a mode, preserve it.
