...
- Record the interface-analysis decision as `profile.designSizeMode`. A viewport-filling interface uses `FillScreen`; `Desired` requires positive evidence that the Widget is local/content-sized, produces non-zero root Desired Size, and is constrained or placed by a host. A `Desired` tree must contain at least one root-direct child with either a point-anchored Canvas `slotLayout` whose `autoSize` is false and whose `offsets.right`/`offsets.bottom` are positive, or a `contentDrivenSize` record with `verified: true`, positive two-axis `measuredDesiredSize`, and a valid `evidenceId`. An empty root, auto-sized fixed Slot, verified-only record, or zero-offset full-stretch-only content is rejected. This general proof does not relax the stricter unique first-Panel rule for `listRole: entry`. `profile.assetKind`, path, and `umg_`/`uw_` naming do not derive this field. When the analysis is unclear, record the `FillScreen` fallback decision. The mode controls the Widget Blueprint Designer `Screen Size` preview; it neither replaces `referenceSize` nor authorizes `DesignTimeSize` writes.
...
- `designSizeMode`: `FillScreen` or `Desired`. New layouts must set the analysis-selected value explicitly. It is independent of `assetKind` and naming. Use `Desired` only with positive local/content-sized evidence; use `FillScreen` for viewport-filling assets and as the `fallback-unclear` default. The `prototype` kind is archived compatibility only and must not be newly emitted; preserve any explicit archived mode, and let a missing legacy/standalone mode enter the planner's recorded `fallback-unclear -> FillScreen` path.
