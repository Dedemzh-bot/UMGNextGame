...
When `analysisPolicy.designSizeModeRequired` is true, do not add a duplicate mode field to `UIBuildBundle.assets`. Derive the expected Widget Blueprint Designer Screen Size only from the linked accepted `assetPlan[].assetKind`: `screen` maps to `profile.designSizeMode: FillScreen`; `child-widget` and `list-entry` map to `profile.designSizeMode: Desired`. With linked-file validation enabled, every layout-backed asset must carry that exact canonical value; a missing value, `DesiredOnScreen`, or any mismatched mode is rejected. The Bundle-to-layout type binding is also exact: `screen` maps to `profile.assetKind: screen`; `child-widget` maps to `profile.assetKind: child-widget` without `listRole: entry`; and `list-entry` maps to `profile.assetKind: child-widget` with `listRole: entry`. A `reuse-only` child/list-entry intentionally has no UILayoutSpec; its expected mode is still derived as `Desired` from the Bundle asset that already exactly mirrors the accepted plan, and final actual Unreal readback must close that verification.
