...
3. **Asset decomposition analyst** -- propose the screen and any explicitly evidenced asset boundaries, dependencies, and build order. Classify every planned asset by semantic scope, not by its name or current Designer default: a complete screen is `screen`, while a local reusable control or collection entry is `child-widget` or `list-entry`. Independently analyze each asset's Designer Screen Size mode and persist a closed `designSizeModeDecision`, bound through `claimId` to one reviewed `asset-decomposition` claim about that asset. Use `FillScreen` with `basis: viewport-filling` when evidence proves viewport-filling behavior; use `Desired` with `basis: content-sized-local` only when positive evidence proves a locally hosted control whose root content produces a non-zero Desired Size. A designated existing Widget may supply a `verified-reference` decision for either mode only when its evidence comes from a `project-asset` source using `locatorKind: unreal-object` and carries `measurementMethod: editor-readback`. If the evidence cannot distinguish these behaviors, choose the safe `FillScreen` fallback with `basis: fallback-unclear`; its proving claim must explicitly record the canonical `fallback-unclear` and `FillScreen` tokens even when the decision has no evidence IDs. Asset prefixes such as `umg_`/`uw_` and `assetKind` are inventory hints only and never decide the mode; `DesiredOnScreen`, `Custom`, and `CustomOnScreen` are outside this contract. For non-Fight systems, keep screen-local regions and collections in the screen; every list-entry, runtime-template, or reusable-widget boundary requires accepted evidence and an inventory classification. Before proposing a new asset for that boundary, compare current-SystemFolder assets and active shared-registry entries by purpose, capabilities, interface, Parent Class, normalized dual-Slot contract, size hard check, state-model hard check, and semantic content similarity. Selected common reuse produces a system-specific Parent Class child; classify every additive visual as below or above the shared base content, map it to `SlotDown` or `SlotUp`, and give each used Slot exactly one direct semantic Panel. Both shared Slots are `SelfHitTestInvisible`; both inherited bindings remain empty when no additive content exists, and a later host/EntryClass relation is recorded separately.
...
- Give every in-scope `assetPlan[]` in the new policy a `designSizeModeDecision` with `mode`, `basis`, a concrete `reason`, canonical `evidenceIds`, and one `claimId`. The claim must exist in the same asset's `claimIds`, use `type: asset-decomposition`, name the asset in `subjectRefs`, and cover every decision evidence ID. `viewport-filling` requires `FillScreen`; `content-sized-local` requires `Desired`; `verified-reference` may support either mode but requires `project-asset` + `unreal-object` Editor readback evidence; `fallback-unclear` requires `FillScreen` and an explicit reviewed fallback claim. Every non-fallback decision, and every `Desired` decision, needs positive evidence attached to that same asset. Never derive this field from `umg_`/`uw_`, `assetKind`, or source-image dimensions.
...
- every in-scope asset has both the correct semantic `assetKind` and an independent evidence-backed `designSizeModeDecision`, with ambiguous cases explicitly defaulted to `FillScreen`;
