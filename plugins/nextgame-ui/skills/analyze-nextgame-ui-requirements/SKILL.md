...
- Treat each accepted in-scope `assetPlan[].assetKind` as the only Designer size-mode decision: `screen` requires `FillScreen`; `child-widget` and `list-entry` require `Desired`. Do not add a second per-asset mode field to the Requirement, and do not choose `DesiredOnScreen` for local controls.
...
- every in-scope asset has the correct semantic `assetKind`, so the downstream Designer Screen Size is unambiguously `FillScreen` for screens or `Desired` for child widgets/list entries;
