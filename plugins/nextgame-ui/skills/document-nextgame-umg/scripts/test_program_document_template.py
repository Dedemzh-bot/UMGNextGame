...
class ProgramDocumentTemplateTest(unittest.TestCase):
    ...
    def test_document_skill_uses_template_without_machine_paths(self) -> None:
        ...
        for required in (
            ...
            "scripts/build_program_docx.py",
            "--document-content <program-document-content.json>",
            "revalidates the current Requirement, Bundle, Unreal Readback, post-build acceptance, Handoff, and content projection",
            "fixed Chinese suffix `系统主界面`",
            "`集合条目控件`",
            "`功能子控件`",
            "0.4 four-column table contract",
            "saved Designer Visibility",
            "requiredSemanticRelationshipStatements",
            "rejects unknown package parts and external relationships",
            "without DEFLATE variability",
            "changed: false",
            "fresh rendering",
        ):
